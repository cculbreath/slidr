    ...
    func importFiles(
        ...
        for (index, url) in urls.enumerated() {
            do {
                ...
                // Skip conversion for reference mode
                let needsVideoConversion = options.importMode != .reference
                    && options.convertIncompatible
                    && VideoConverter.needsConversion(url: url)
                ...
                // Reference mode: no copy, store absolute path
                if options.importMode == .reference {
                    let item = MediaItem(
                ...
                // Build destination path based on storage location
                ...
                case .external:
                    ...
                    if options.organizeByDate {
                        ...
                        relativePath = "\(year)/\(String(format: "%02d", month))/\(uuid).\(finalExtension)"
                    } else {
                        relativePath = "\(year)/\(uuid).\(finalExtension)"
                    }
                    destinationURL = extRoot.appendingPathComponent(relativePath)
                    storageLocation = .external

                case .local, .referenced:
                    if options.organizeByDate {
                        let month = Calendar.current.component(.month, from: dateForOrg)
                    ...
                    destinationURL = libraryRoot.appendingPathComponent(relativePath)
                ...
                // Move mode: delete the original
                if options.importMode == .move {
...
enum ImportError: LocalizedError {
    ...
    case hashingFailed
    case copyFailed
    case externalDriveNotAvailable
...
struct ImportResult: Sendable {
    ...
    mutating func merge(_ other: ImportResult) {
        ...
        skippedDuplicates.append(contentsOf: other.skippedDuplicates)
        converted.append(contentsOf: other.converted)
