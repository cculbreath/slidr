...
actor TranscriptStore {
    ...
    /// Saves SRT content generated by transcription services.
    /// Returns parsed cues, plain text, and the relative path for storage.
    func saveTranscript(
        srtContent: String,
        forContentHash contentHash: String
    ) throws -> (cues: [TranscriptCue], plainText: String, relativePath: String) {
        let destinationFilename = "\(contentHash).srt"
        let destinationURL = transcriptDirectory.appendingPathComponent(destinationFilename)
        ...
        try srtContent.write(to: destinationURL, atomically: true, encoding: .utf8)
        ...
        Logger.transcripts.info("Saved transcription for \(contentHash): \(cues.count) cues")
